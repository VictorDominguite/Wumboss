...
public class CelulaFactory {
	...
	public static ICelula montar(Sala sala, int x, int y, String repr) {
		Celula c = new Celula(x, y, decodeRawEntity(repr));
		EntidadeDinamica e = ForegroundFactory.decodeRawEntity(repr);
		if (e != null) {
			e.connect(Space.getInstance());
			c.pushEntidade(e);
			if(e.isInimigo())
				sala.inimigoAdicionado((IInimigo) e);
		}
		return c;
	}
}
